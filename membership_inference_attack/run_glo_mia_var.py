...
def main(dataset_name, defense, sigma=None):    
    ...
    # Run multiple times per dataset split to minimize model initialization randomness
    for i in range(NUM_RUNS):
        print(f'Run #{i+1}')
        t = time.perf_counter()
        # Train target model
        t_save_path = f'mia-models/t_model_gat_{dataset_name}_{i}.pth'
        t_model = train_gat(dataset_name, 't', t_dataset_train, dataset_test=t_dataset_test, save_path=t_save_path, device=DEVICE, verbose=0)
        # Train shadow model
        s_save_path = f'mia-models/s_model_gat_{dataset_name}_{i}.pth'
        s_model = train_gat(dataset_name, 's', s_dataset_train, dataset_test=s_dataset_test, save_path=s_save_path, device=DEVICE, verbose=0)
        ...
        print(f'Run {time.perf_counter()-t:.4f}s')
