...
def metric(ori_adj, inference_adj, idx):
    ...
    precisions, recalls, _ = precision_recall_curve(y_true_b, y_score_b)
    # last PR point corresponds to threshold=+inf → skip for F1
...
class NoisyModel(torch.nn.Module):
    """
    Generic wrapper that adds Gaussian noise to the outputs of a base module.

    - Works for victim classifier AND embedding networks.
    - Assumes base_module(*args, **kwargs) -> tensor of shape (N, C) or (N, D).
    - Copies over key attributes (nclass, nfeat, hidden_sizes, device) so that
      BaseAttack / PGDAttack can still read them.
    """

    def __init__(self, base_module, sigma: float):
        super().__init__()
        self.base_module = base_module
        self.sigma = float(sigma)

        # Copy attributes that BaseAttack expects to find on the model
        for attr in ["nclass", "nfeat", "hidden_sizes", "device"]:
...
def run_single_experiment(
    ...
    """Train victim, run attack (with optional logit noise), and return metrics."""
    ...
    # Start from clean models
    defended_model = victim_model
    defended_embedding = embedding

    # Optional defense: add Gaussian noise to outputs
    if args.defense == "logit_noise" and args.defense_sigma > 0.0:
        print(f"Using logit+embedding-noise defense with sigma={args.defense_sigma}")
        defended_model = NoisyModel(victim_model, sigma=args.defense_sigma).to(device)
        defended_embedding = NoisyModel(embedding, sigma=args.defense_sigma).to(device)

    attack = PGDAttack(
        model=defended_model,
        embedding=defended_embedding,
        nnodes=adj.shape[0],
        loss_type="CE",
        device=device,
    ).to(device)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "logit_noise"],
        help="Defense type to apply during attack.",
    )
    parser.add_argument(
        "--defense_sigma",
        type=float,
        default=0.05,
        help="Stddev of Gaussian noise for defenses (e.g., logit noise).",
    )
    ...
    summary = (
        df.groupby(["dataset", "arch", "defense", "defense_sigma"])[
            ["test_acc", "attack_auc", "attack_ap", "attack_f1"]
        ].agg(["mean", "std"])
    )
    print("\n=== Summary by (dataset, arch, defense, sigma) (mean ± std) ===")
    print(summary)

    os.makedirs(args.outdir, exist_ok=True)
    raw_path = os.path.join(args.outdir, "graphmi_experiments_raw_with_def_2.csv")
    summary_path = os.path.join(args.outdir, "graphmi_experiments_summary_with_def_2.csv")
