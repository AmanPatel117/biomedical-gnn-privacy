...
def get_train_val_test_gcn(labels, seed=None):
    ...
    for i in range(nclass):
        ...
        idx_train = np.hstack((idx_train, labels_i[: 20])).astype(np.int64)
        idx_unlabeled = np.hstack((idx_unlabeled, labels_i[20: ])).astype(np.int64)
...
def get_splits_each_class(labels, train_size):
    ...
    for i in range(nclass):
        labels_i = idx[labels==i]
        labels_i = np.random.permutation(labels_i)
        idx_train = np.hstack((idx_train, labels_i[: train_size])).astype(np.int64)
        idx_val = np.hstack((idx_val, labels_i[train_size: 2*train_size])).astype(np.int64)
        idx_test = np.hstack((idx_test, labels_i[2*train_size: ])).astype(np.int64)
